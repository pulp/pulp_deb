...
cd $TRAVIS_BUILD_DIR
export REPORTED_VERSION=$(http :24817/pulp/api/v3/status/ | jq --arg plugin pulp_deb -r '.versions[] | select(.component == $plugin) | .version')
...
GEM_FILE="$(ls | grep pulp_deb_client-)"
gem push ${GEM_FILE}
