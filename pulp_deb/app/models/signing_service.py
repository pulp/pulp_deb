            ...
            try:
                with open(test_release_path, "wb") as test_file:
                    ...
                    if imported_keys[0]["fingerprint"] != self.pubkey_fingerprint:
                        raise RuntimeError(
                            "The signing service fingerprint does not match its public key!"
                        )
                    ...
                    if inline_path:
                        ...
                        # Also check that the non-signature part of the InRelease file is the same
                        # as the original Release file!
                        with open(inline_path, "rb") as inline:
                            ...
                            if test_data not in inline_data:
                                message = (
                                    "The inline file '{}' contains different data from the "
                                    "original file."
                                )
                                raise RuntimeError(message.format(inline_path))
                    ...
                    if detached_path:
                        if os.path.basename(detached_path) != "Release.gpg":
                            message = (
                                "The path returned via the 'signatures.detached' field of the "
                                "signing service script, must end with the 'Release.gpg' file name!"
                            )
                            raise RuntimeError(message)
